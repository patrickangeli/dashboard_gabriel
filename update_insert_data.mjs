...
const insertDataCode = `import React, { useState, useEffect } from 'react';
...
export default function InsertData() {
  ...
  const handleProjetoSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:3001/api/projetos', {
  ...
  const handlePrevisaoSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:3001/api/previsao', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (res.ok) {
...
fs.writeFileSync('src/pages/InsertData.jsx', insertDataCode);
console.log('InsertData.jsx updated successfully!');
